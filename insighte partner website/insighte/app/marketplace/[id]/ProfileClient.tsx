...
export default function ProfileClient({ provider }: { provider: any }) {
  ...
  const price = provider.rate || (services?.[0]?.price) || "450";
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-16">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="relative w-full aspect-square max-w-[320px] mx-auto lg:mx-0 rounded-3xl overflow-hidden border-2 border-white/5 bg-[#191a2d]">
            <Image
              src={provider.profile_image || "https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=800&auto=format&fit=crop"}
              fill
              alt={provider.name}
        ...
        <div className="lg:col-span-8 flex flex-col justify-center space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-[#c8c5cd]">
            ...
            <span className="flex items-center gap-1.5">
              <Check className="w-4 h-4 text-[#baccb3]" /> 300+ children supported
            </span>
            <span className="flex items-center gap-1.5">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 pt-4 border-t border-white/5">
            ...
            <div className="flex items-center gap-3 flex-1 w-full sm:w-auto">
              <Link href={`/book?provider_id=${provider.id}`} className="flex-1 sm:flex-initial">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          {/* About */}
          <div className="bg-[#191a2d]/60 rounded-2xl border border-white/5 p-8">
            <h2 className="text-xl font-bold font-manrope text-white mb-4">About</h2>
            <p className="text-[#c8c5cd] leading-relaxed">
              {provider.bio || `${provider.name} is a dedicated specialist who works with children and families using evidence-based, neuro-affirming methods. With years of experience in ${provider.category || "child development"}, they create safe, engaging environments where every child can grow at their own pace.`}
            </p>
          </div>

          {/* Approach */}
          <div className="bg-[#191a2d]/60 rounded-2xl border border-white/5 p-8">
            <h2 className="text-xl font-bold font-manrope text-white mb-4">Approach</h2>
            <ul className="space-y-3 text-[#c8c5cd]">
              ...
              <li className="flex items-start gap-3">
                <Check className="w-4 h-4 text-[#baccb3] mt-1 shrink-0" />
                <span>Family-inclusive approach — parents are partners, not spectators</span>
              </li>
            </ul>
          </div>
          ...
          {/* Parent Outcomes */}
          <div className="bg-[#191a2d]/60 rounded-2xl border border-white/5 p-8">
            <h2 className="text-xl font-bold font-manrope text-white mb-4">What parents report after sessions</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                ...
                <div key={i} className="flex items-start gap-2.5 text-sm text-[#c8c5cd]">
                  <div className="w-5 h-5 rounded-full bg-[#baccb3]/10 flex items-center justify-center shrink-0 mt-0.5">
                    <Check className="w-3 h-3 text-[#baccb3]" />
                  </div>
                  <span>{outcome}</span>
                </div>
              ))}
              ...
              {(provider.education || []).length > 0 ? (
                provider.education.map((edu: any, i: number) => (
                  <div key={i} className="border-l-2 border-[#baccb3]/20 pl-4 space-y-0.5">
                    ...
                    <p className="text-xs text-[#c8c5cd]">{edu.institution} • {edu.year}</p>
                  </div>
                ))
              ) : (
          ...
          <Accordion title="Work Experience" icon={<Briefcase className="h-4 w-4" />}>
            <div className="space-y-4 pt-3">
              {(provider.work_experience || []).length > 0 ? (
                provider.work_experience.map((work: any, i: number) => (
                  <div key={i} className="border-l-2 border-[#d3c4b5]/20 pl-4 space-y-0.5">
                    <p className="text-sm font-bold text-white">{work.role}</p>
        ...
        <div className="lg:col-span-5 space-y-6">
              ...
              {filteredServices.length === 0 ? (
                ...
                filteredServices.map((service: any) => (
                  <div key={service.id} className="bg-white/5 rounded-xl p-5 border border-white/5 hover:border-[#d3c4b5]/20 transition-all group">
                    ...
                    <Link href={`/book?provider_id=${provider.id}&service_id=${service.id}`}>
          ...
          <div className="bg-gradient-to-br from-[#d3c4b5]/10 to-[#baccb3]/10 rounded-2xl border border-[#d3c4b5]/20 p-8 text-center space-y-4">
            ...
            <Link href={`/book?provider_id=${provider.id}`}>
            ...
            <div className="bg-[#191a2d]/60 rounded-2xl border border-white/5 p-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {slots.map((slot: any) => {
                  ...
                  return (
                    <Link key={slot.id} href={`/book?provider_id=${provider.id}&slot_id=${slot.id}`}>
